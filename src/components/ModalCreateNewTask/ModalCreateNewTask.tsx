...
interface Props {
    setIsShowModalCreateNewTask: (status: boolean) => void
}

export default function ModalCreateNewTask(props: Props) {
    ...
    return (
        <div className='w-screen h-screen absolute top-0 left-0 flex items-start justify-center bg-black bg-opacity-50 pt-20'>
            <div className='w-[550px] relative bg-white rounded-xl p-10'>
                <IoCloseSharp className='absolute top-3 right-3 text-xl text-gray-600 cursor-pointer hover:text-black' onClick={() => props.setIsShowModalCreateNewTask(false)} />
                <form onSubmit={() => { }} className='space-y-3'>
                    <div className='flex justify-center'>
                        <p className='text-[22px] font-bold'>CREATE NEW TASK</p>
                    </div>
                    ...
                    <div className='flex items-center justify-center pt-4'>
                        <button className='px-12 py-2 bg-blue-500 rounded-md text-white text-xl font-bold'>Create task</button>
                    </div>
                </form>
            </div>
        </div>
